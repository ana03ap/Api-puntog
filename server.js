...
// Endpoint para suscribirse a un evento (decrementa un cupo)
...
// Endpoint para enviar feedback anónimo
app.post('/feedback/:id', (req, res) => {
  const id = parseInt(req.params.id);
  const { rating, comment } = req.body;
  const event = events.find(e => e.id === id);
  if (event) {
    event.feedback.push({ rating, comment });
    res.json({ message: "Feedback submitted!", feedback: event.feedback });
  } else {
